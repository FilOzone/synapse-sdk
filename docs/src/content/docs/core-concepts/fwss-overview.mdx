...
4. **Metadata Management**: Stores data set metadata and emits piece metadata for off-chain indexing.
...
When pieces are added to an existing data set, this function validates and emits piece metadata and updates payment rail configuration if needed. Piece metadata is available through `PieceAdded` events or an indexer, not contract getters.

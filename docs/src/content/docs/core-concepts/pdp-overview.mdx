...
This page explains **Proof of Data Possession (PDP)**, the cryptographic protocol which ensures every dataset stored within the **Filecoin Onchain Cloud (FOC)** remains accessible, verifiable, and provably intact without re-uploading the full data.
...
2. **Challenge Creation**: The PDP contract generates a randomized challenge based on the drand beacon and dataset ID.

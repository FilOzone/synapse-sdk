...
```ts twoslash
import { Synapse, RPC_URLS, TOKENS, WarmStorageService, CONTRACT_ADDRESSES } from '@filoz/synapse-sdk'
import { ethers } from 'ethers'
...
const GB_IN_BYTES = 1024 * 1024 * 1024 // 1GB
const storageNeeded = 10 * GB_IN_BYTES // 10GB
const response = await warmStorageService.checkAllowanceForStorage(storageNeeded, false, synapse.payments)
const requiredAmount = response.depositAmountNeeded

// Approve only if needed
if (response.sufficient === false) {

...
Then upload and download data with `pynapse`:
...
async def main() -> None:
    ...
    data = (b"Hello from pynapse on Filecoin Calibration! " * 8)
    result = await synapse.storage.upload(data)
    print(f"Uploaded: {result.piece_cid} ({result.size} bytes)")

    downloaded = await synapse.storage.download(result.piece_cid)
...
Note: `pynapse` requires the external `stream-commp` helper at runtime for PieceCID calculation. See the [pynapse README](https://github.com/anjor/pynapse#commp--piececid).
...
const depositAmount = parseUnits("2.5") // 2.5 USDFC

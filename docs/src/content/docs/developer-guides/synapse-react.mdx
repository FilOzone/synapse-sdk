...
<Tabs syncKey="pkg">
  ...
  <TabItem label="pnpm" icon="pnpm">
    ```bash
    pnpm add @filoz/synapse-react wagmi viem @tanstack/react-query
    ```
  </TabItem>
  <TabItem label="yarn" icon="seti:yarn">
    ```bash
    yarn add @filoz/synapse-react wagmi viem @tanstack/react-query
    ```
  </TabItem>
  <TabItem label="bun" icon="bun">
    ```bash
    bun add @filoz/synapse-react wagmi viem @tanstack/react-query
    ```
  </TabItem>
</Tabs>
...
:::caution[Piece metadata]
Each data set returned by `useDataSets()` has a `pieces: Piece[]` array whose items have no `metadata` field. Read piece metadata from FWSS `PieceAdded` events or an indexer instead. See the [migration guide](/developer-guides/migration-guide/#action-replace-core-piece-metadata-reads) for the core API changes.
:::

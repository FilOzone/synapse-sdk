...
:::caution[Deprecated]
The `metadata` field returned for each piece by `useDataSets()` depends on deprecated FWSS piece metadata getters and will be removed. Read piece metadata from `PieceAdded` events or an indexer instead.
:::

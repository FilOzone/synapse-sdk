...
console.log(pieceCid.size) // size of the piece in bytes
...
The `endorsedIds` parameter controls which providers are eligible. When non-empty, **only** endorsed providers can be selected (no fallback to non-endorsed). When empty, all approved providers are eligible. See [Storage Provider Tiers](/core-concepts/storage-providers) for what approval and endorsement mean and how providers qualify.
...
if (!secondaryProvider) throw Error('not found')
if (!primaryProvider) throw Error('not found')

...
import { privateKeyToAccount } from "viem/accounts"
import { calibration } from "@filoz/synapse-core/chains"
const account = privateKeyToAccount("0x1")
const walletClient = createWalletClient({ account, chain: calibration, transport: http() })
...
// 1. Select a provider
const provider = await getPDPProvider(walletClient, { providerId: 1n })
...
console.log("Creating data set and adding piece")
...
console.log(confirmed.dataSetId)  // bigint — on-chain data set ID
console.log(confirmed.piecesIds)  // bigint[] — confirmed piece IDs
```

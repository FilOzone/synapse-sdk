...
Legacy data sets have an **80-piece gas safety cap** in addition to the SDK's encoded message-size check. Compact data sets and new-data-set creation use the same size check without that legacy cap. Provider limits and transaction gas can further reduce batch size. [Legacy batching limit](https://github.com/FilOzone/synapse-sdk/issues/932)

:::note[The SDK handles batching]
The SDK currently splits compatible concurrent uploads into batches of **at most 40 pieces** for Curio compatibility. This temporary provider cap applies to both legacy and compact data sets and takes precedence over the 80-piece legacy cap. Larger batches require a provider update and a corresponding SDK change. You can start uploads together without manually grouping them. [Provider support](https://github.com/filecoin-project/curio/issues/1421)
:::
...
The SDK prefers matching compact data sets over legacy ones during automatic and explicit provider selection. If no compact data set matches, it can reuse a legacy one. To keep the same metadata while switching, create replacements with the core [`createDataSet()` and `waitForCreateDataSet()` APIs](https://github.com/FilOzone/synapse-sdk/blob/95707ac015f547ea1ba1181c87e2f66504cd4895/packages/synapse-core/src/sp/create-dataset.ts), then reuse them. Passing `dataSetIds` explicitly selects those data sets regardless of storage format. Selection does not move existing content.

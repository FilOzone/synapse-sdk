...
| **CDN Topup**  | 1 USDFC (one-time) | Pre-paid egress credits, not a fee. Reusing an existing dataset incurs no additional cost |
...
- CDN datasets require 1 USDFC topup on first creation only
...
Determine lockup and rate allowances, including CDN costs if creating a new dataset:
...
// Check if creating a new CDN dataset

...
The SDK automatically splits compatible concurrent uploads according to the encoded Filecoin message-size budget. Legacy data sets additionally have an 80-piece safety cap. Start uploads together without manually grouping them or waiting between groups. See [Larger, Cheaper Storage Batches](/developer-guides/storage/storage-upgrade/#more-pieces-per-operation) for details.

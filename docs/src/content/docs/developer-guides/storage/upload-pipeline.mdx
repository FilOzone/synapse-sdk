...
The SDK automatically splits compatible concurrent uploads into batches of at most **40 pieces** for Curio compatibility, also checking encoded message size. Legacy data sets have an additional 80-piece safety cap that becomes relevant when provider support allows larger batches. Start uploads together without manually grouping them or waiting between groups. See [Larger, Cheaper Storage Batches](/developer-guides/storage/storage-upgrade/#more-pieces-per-operation) for the limits and provider requirements.

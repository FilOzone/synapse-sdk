...
The SDK automatically splits compatible concurrent uploads into batches of at most 40 pieces. Start uploads together without manually grouping them or waiting between groups. See [Larger, Cheaper Storage Batches](/developer-guides/storage/storage-upgrade/#more-pieces-per-operation) for the gas benefits and larger capacity enabled by the contracts.
...
By default, the SDK submits a batch after a zero-delay window once its uploads and pulls finish parking. You can instead hold compatible pieces until the next piece would exceed the 40-piece cap or encoded message-size budget, or until you explicitly flush them:
...
The default is 2 copies. The first copy is stored on an [**endorsed** provider](/core-concepts/storage-providers#endorsed-providers) (high trust, curated), and secondary copies are stored with [approved providers](/core-concepts/storage-providers#approved-providers), which pull the data from the primary via SP-to-SP transfer.

...
To benefit from compact piece storage, explicitly switch future uploads to [new data sets](/developer-guides/storage/storage-upgrade/#choose-new-data-sets-once-then-reuse-them), then reuse them. Matching an old data set continues using its legacy piece storage; the SDK does not migrate it automatically.
...
- Otherwise, the SDK selects from [endorsed providers](/core-concepts/storage-providers#endorsed-providers) for the primary copy and any [approved provider](/core-concepts/storage-providers#approved-providers) for secondaries

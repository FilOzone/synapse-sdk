...
After the storage contract upgrade, use distinct metadata to select new [compact data sets](/developer-guides/storage/storage-upgrade/), then reuse them. Matching an old data set continues using its legacy piece storage; the SDK does not migrate it automatically.

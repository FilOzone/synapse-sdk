...
:::caution[Deprecated]
Piece metadata getters are deprecated and will be removed from Synapse SDK packages. Read metadata from `PieceAdded` events or an indexer instead. Passing piece metadata when uploading or committing remains supported so providers can include it in those events.
:::

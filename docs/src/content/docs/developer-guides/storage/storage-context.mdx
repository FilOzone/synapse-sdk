...
type PullStatus = { status: string };
export interface UploadCallbacks {
  /** Called periodically during upload with bytes uploaded so far */
  onProgress?: (bytesUploaded: number) => void;
  /** Called when data has been uploaded to a provider */
  onUploadComplete?: (pieceCid: PieceCID) => void;
  /** Called after data is stored on a provider */
  ...
  /** Called when a copy to a secondary provider completes */
...
const { pieceCid, size, pieceId } = await storageContext.upload(data, {
  ...
  onStored: (providerId, piece) => {
    console.log(`Stored on provider ${providerId}: ${piece.toString()}`);
  },
  onPieceAdded: (providerId, piece) => {
    console.log(
      `Piece submitted for on-chain addition on provider ${providerId}: ${piece.toString()}`
    );
  },
  onPieceConfirmed: (providerId, piece, pieceId) => {
    console.log(
      `Piece ${pieceId} confirmed on provider ${providerId}: ${piece.toString()}`
    );
  },
});

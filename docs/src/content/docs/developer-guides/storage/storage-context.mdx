...
type Address = `0x${string}`
type Hex = `0x${string}`;
type UploadPieceStreamingData = Uint8Array | ReadableStream<Uint8Array> | File | Blob
 ...
 * @param pieceCid - Optional pre-calculated PieceCID to skip CommP calculation
...
interface StorageContextAPI {
  ...
  // Piece Queries
  hasPiece(options: { pieceCid: string | PieceCID }): Promise<boolean>;
  pieceStatus(options: { pieceCid: string | PieceCID }): Promise<PieceStatus>;
  getPieces(options?: { batchSize?: bigint }): AsyncGenerator<PieceRecord>;

  // Piece Management
  deletePiece(options: { piece: string | PieceCID | bigint }): Promise<Hash>;

  // Info & Preflight
  getProviderInfo(): Promise<PDPProvider>;
  preflightUpload(options: { size: number }): Promise<PreflightInfo>;
...
// Get all pieces in the current data set by querying the provider
const pieces = await Array.fromAsync(storageContext.getPieces());
console.log(`Piece CIDs: ${pieces.map((piece) => piece.pieceCid.toString()).join(", ")}`);
...
const pieces = [] as Uint8Array[]
// ---cut---
// ✅ Efficient: Batched automatically
const uploads = pieces.map((piece) => context.upload(piece));
const results = await Promise.all(uploads);

// ❌ Slow: Forces sequential processing
for (const piece of pieces) {
  await context.upload(piece);
}
```
...
To delete an individual piece from the data set, call `context.deletePiece({ piece: pieceCid })`.
...
const dataFromProvider = await synapse.storage.download({
  pieceCid,
  providerAddress: "0x...",
});
```
...
const synapse = Synapse.create({ withCDN: true }); // Global default: CDN enabled

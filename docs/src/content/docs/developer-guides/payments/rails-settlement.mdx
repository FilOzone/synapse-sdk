...
async function settleAllIncomingRails() {
  ...
  for (const rail of rails) {
    try {
      ...
      const threshold = parseUnits("10", 18); // 10 USDFC
      if (amounts.totalNetPayeeAmount > threshold) {
        ...
        await waitForTransactionReceipt( client, { hash });
        console.log(
          `Settled rail ${rail.railId} for ${formatUnits(amounts.totalNetPayeeAmount, 18)} USDFC`
        );
      }
    } catch (error) {
...
async function prepareForWithdrawal() {
  ...
  const availableBalance = (await synapse.payments.accountInfo())
    .availableFunds;
  console.log(
    `Available for withdrawal: ${formatUnits(availableBalance, 18)} USDFC`
  );
}
```

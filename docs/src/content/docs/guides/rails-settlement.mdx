...
#### 1. The Lockup Guarantee
When a rail is created, the system calculates a lockup requirement:
...
- **Key Concept**: This amount acts as a **minimum balance floor** - you cannot withdraw below it
...
- If funds are insufficient for settlement, the transaction fails and may trigger rail termination
...
The lockup serves as a protection for service providers:
- If a payer stops depositing and becomes insolvent, settlement stops for new epochs
- When a rail is terminated, the lockup becomes a **guaranteed payment window**
- This ensures providers can claim payments for the lockup period even if the payer disappears

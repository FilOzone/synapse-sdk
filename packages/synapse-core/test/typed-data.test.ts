...
const FIXTURES = {
  ...
  signatures: {
    ...
    addPieces: {
      extraData:
        '' as Hex,
      clientDataSetId: 12345n,
      nonce: 1n,
    },
...
describe('Typed Data', () => {
  ...
  it('should retain per-piece metadata arrays for a mixed batch', async () => {
    const extraDataActual = await TypedData.signAddPieces(client, {
      clientDataSetId: FIXTURES.signatures.addPieces.clientDataSetId,
      nonce: FIXTURES.signatures.addPieces.nonce,
      pieces: PIECE_DATA.map((piece, index) => ({
        pieceCid: Piece.from(piece),
        metadata: index === 1 ? [{ key: 'title', value: 'TestDataSet' }] : undefined,
      })),
    })

    const decoded = decodeAbiParameters(TypedData.signAddPiecesAbiParameters, extraDataActual)

    assert.deepStrictEqual(decoded[1], [[], ['title']])
    assert.deepStrictEqual(decoded[2], [[], ['TestDataSet']])
  })

...
describe('Pull', () => {
  ...
  const baseOptions = (): SP.pullPieces.OptionsType => ({
  ...
  describe('pullPieces', () => {
    it('should handle successful pull request', async () => {
      ...
      const result = await SP.pullPieces(baseOptions())
    ...
    it('should send correct request body', async () => {
      ...
      await SP.pullPieces(baseOptions())
    ...
    it('should include dataSetId when provided', async () => {
      ...
      await SP.pullPieces({ ...baseOptions(), dataSetId: 123n })
    ...
    it('should not include dataSetId when zero', async () => {
      ...
      await SP.pullPieces({ ...baseOptions(), dataSetId: 0n })
    ...
    it('should handle mixed piece statuses', async () => {
      ...
      const mockResponse: SP.PullResponse = {
        status: 'inProgress',
        pieces: [
      ...
      const result = await SP.pullPieces({
        ...baseOptions(),
        pieces: [
  ...
  describe('waitForPullStatus', () => {
    it('should poll until complete', async () => {
      ...
      const statusUpdates: SP.PullStatus[] = []
    ...
    it('should stop polling on failed status', async () => {
      ...
      const result = await SP.waitForPullStatus({ ...baseOptions(), minTimeout: 10 })
    ...
    it('should handle server errors during polling', async () => {
      ...
      try {
        await SP.waitForPullStatus({ ...baseOptions(), minTimeout: 10 })
        assert.fail('Should have thrown error')
      } catch (error) {
        assert.ok(error instanceof PullError, 'Error should be PullError')
      }

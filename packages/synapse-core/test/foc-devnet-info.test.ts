...
describe('foc-devnet-info', () => {
  ...
  describe('loadDevnetInfo', () => {
    ...
    it('should validate user information', () => {
      const result = loadDevnetInfo(exampleDevnetInfoV1)
      const user1 = result.info.users[0]

      assert.equal(user1.name, 'USER_1')
      assert.ok(user1.evm_addr.startsWith('0x'))
      assert.ok(user1.native_addr.startsWith('t410f'))
      assert.ok(user1.private_key_hex.startsWith('0x'))
    })

    it('should validate contract addresses', () => {
      const result = loadDevnetInfo(exampleDevnetInfoV1)
      const { contracts } = result.info

      assert.ok(contracts.multicall3_addr.startsWith('0x'))
      assert.ok(contracts.mockusdfc_addr.startsWith('0x'))
      assert.ok(contracts.fwss_service_proxy_addr.startsWith('0x'))
      assert.ok(contracts.pdp_verifier_proxy_addr.startsWith('0x'))
      assert.ok(contracts.service_provider_registry_proxy_addr.startsWith('0x'))
      assert.ok(contracts.filecoin_pay_v1_addr.startsWith('0x'))
      assert.ok(contracts.endorsements_addr.startsWith('0x'))
      assert.ok(contracts.session_key_registry_addr.startsWith('0x'))
    })

    it('should throw on invalid data', () => {
      assert.throws(() => {
        loadDevnetInfo({ version: 1, info: {} })
      }, /DevNet info schema validation failed/)
    })

    it('should throw on missing version', () => {
      assert.throws(() => {
        loadDevnetInfo({ info: exampleDevnetInfoV1.info })
      }, /DevNet info schema validation failed/)
    })

    it('should throw on wrong version', () => {
      assert.throws(() => {
        loadDevnetInfo({ version: 2, info: exampleDevnetInfoV1.info })
      }, /DevNet info schema validation failed/)
    })
  })

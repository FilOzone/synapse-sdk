...
type BasePullPiecesOptions = {
  ...
  /** Optional nonce for the add pieces signature. */
  nonce?: bigint
...
export namespace pullPieces {
  ...
  export type ReturnType = Pull.PullResponse

  export type ErrorType =
    | Pull.fetchPieces.ErrorType
 ...
 */
function toPullPieces(pieces: PullPieceInput[]): Pull.PullPieceInput[] {
  return pieces.map((piece) => ({
 ...
 *
 * This function handles EIP-712 signing for authorization and calls the
 ...
 *
 * @example Pulling pieces into an existing data set:
 * ```ts
 * const response = await pullPieces(client, {
 *   endpoint: 'https://sp.example.com',
 *   dataSetId: 123n,
 *   clientDataSetId: dataSet.clientDataSetId,
 *   pieces: [{
 *     pieceCid: parsedPieceCid,
 *     sourceUrl: 'https://source-sp.example.com/piece/bafk...',
 *   }],
 * })
 * ```
 *
 * @example Creating a new data set and pulling pieces:
 * ```ts
 * const response = await pullPieces(client, {
 *   endpoint: 'https://sp.example.com',
...
export async function pullPieces(
  ...
  const recordKeeper = options.recordKeeper ?? chain.contracts.storage.address

  let extraData: Hex
  let dataSetId: bigint | undefined

  if (isExistingDataSet(options)) {
    // Adding to existing data set - use signAddPieces
    ...
    // Creating new data set - use signCreateDataSetAndAddPieces
    ...
    // dataSetId stays undefined for new data set
  }

  return Pull.fetchPieces({
...
export namespace waitForPullStatus {
  ...
  export type OptionsType = pullPieces.OptionsType & {
    ...
    onStatus?: (response: Pull.PullResponse) => void
...
export async function waitForPullStatus(
  ...
  return Pull.waitForFetchStatus({
    endpoint: options.endpoint,
    recordKeeper,
    extraData,
    dataSetId,

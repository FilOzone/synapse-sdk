...
export namespace pullPieces {
  ...
  export type ReturnType = SP.PullResponse

  export type ErrorType =
    | SP.pullPieces.ErrorType
 ...
 */
function toPullPieces(pieces: PullPieceInput[]): SP.PullPieceInput[] {
  return pieces.map((piece) => ({
...
async function resolvePullParams(
  client: Client<Transport, Chain, Account>,
  options: pullPieces.OptionsType
): Promise<SP.pullPieces.OptionsType> {
...
export async function pullPieces(
  ...
  return SP.pullPieces(await resolvePullParams(client, options))
}
...
export namespace waitForPullStatus {
  ...
  export type OptionsType = pullPieces.OptionsType & {
    ...
    onStatus?: (response: SP.PullResponse) => void

...
import type { ApprovedProviderInfo, PieceLink, UploadResult } from '../types.js'
...
const mockSynapse = {
  ...
  download: async (pieceLink: string | PieceLink, options?: any) => {
...
describe('StorageService', () => {
  ...
  describe('download', () => {
    it('should download and verify a piece', async () => {
      ...
      const testPieceLink = 'bafkzcibeqcad6efnpwn62p5vvs5x3nh3j7xkzfgb3xtitcdm2hulmty3xx4tl3wace'

      // Create a mock Synapse with custom download
      const mockSynapseWithDownload = {
        ...mockSynapse,
        download: async (pieceLink: string | PieceLink, options?: any) => {
          assert.equal(pieceLink, testPieceLink)
      ...
      const downloaded = await service.download(testPieceLink)
    ...
    it('should handle download errors', async () => {
      ...
      try {
        await service.download(testPieceLink)
    ...
    it('should accept empty download options', async () => {
      ...
      const downloaded1 = await service.download(testPieceLink)
      assert.deepEqual(downloaded1, testData)

      const downloaded2 = await service.download(testPieceLink, {})
  ...
  describe('upload', () => {
    ...
    it('should support parallel uploads', async () => {
      ...
      let nextPieceId = 0
      const addPiecesCalls: Array<{ pieceLink: string, pieceId: number }> = []
      ...
      serviceAny._pdpServer.uploadPiece = async (data: Uint8Array): Promise<any> => {
        // Use the first byte to create a unique pieceLink for each upload
        const pieceLink = `baga6ea4seaqao7s73y24kcutaosvacpdjgfe5pw76ooefnyqw4ynr3d2y6x2m${data[0]}`
        return { pieceLink, size: data.length }
      }
      ...
      serviceAny._pdpServer.addPieces = async (dataSetId: number, clientDataSetId: number, nextPieceId: number, comms: Array<{ cid: { toString: () => string } }>): Promise<any> => {
        ...
        comms.forEach((comm, index) => {
          addPiecesCalls.push({ pieceLink: comm.cid.toString(), pieceId: nextPieceId + index })
        })
      ...
      const uploads = [
        service.upload(firstData, {
          onUploadComplete: (pieceLink: PieceLink) => uploadCompleteCallbacks.push(pieceLink.toString()),
      ...
      for (const result of results) {
        assert.isTrue(
          addPiecesCalls.some(call => call.pieceLink === result.pieceLink.toString() && call.pieceId === result.pieceId),
          `addPieces call for pieceLink ${String(result.pieceLink)} and pieceId ${result.pieceId != null ? String(result.pieceId) : 'not found'} should exist`
        )
      }
    ...
    it('should handle batch size of 1', async () => {
      ...
      // Mock PDPServer methods
      serviceAny._pdpServer.uploadPiece = async (data: Uint8Array): Promise<any> => ({
        pieceLink: `baga6ea4seaqao7s73y24kcutaosvacpdjgfe5pw76ooefnyqw4ynr3d2y6x2m${data[0]}`,
        size: data.length
      })
      serviceAny._pdpServer.findPiece = async (): Promise<any> => ({ uuid: 'test-uuid' })
    ...
    it('should accept data at exactly 65 bytes', async () => {
      ...
      serviceAny._pdpServer.uploadPiece = async (data: Uint8Array): Promise<any> => {
        ...
        return { pieceLink: testPieceLink, size: data.length }
      }
      ...
      assert.equal(result.pieceLink.toString(), testPieceLink)
    ...
    it('should handle upload callbacks correctly', async () => {
      ...
      serviceAny._pdpServer.uploadPiece = async (): Promise<any> => {
        return { pieceLink: testPieceLink, size: testData.length }
      }
      ...
      const result = await service.upload(testData, {
        onUploadComplete: (pieceLink: PieceLink) => {
          assert.equal(pieceLink.toString(), testPieceLink)
  ...
  describe('pieceStatus()', () => {
    const mockPieceLink = 'bafkzcibeqcad6efnpwn62p5vvs5x3nh3j7xkzfgb3xtitcdm2hulmty3xx4tl3wace'
    ...
    it('should return exists=false when piece not found on provider', async () => {
      ...
      const status = await service.pieceStatus(mockPieceLink)
    ...
    it('should return piece status with proof timing when piece exists', async () => {
      ...
      serviceAny._pdpServer.getDataSet = async () => ({
        ...
        pieces: [{
          pieceId: 1,
          pieceCid: { toString: () => mockPieceLink }
        }],
        nextChallengeEpoch: 5000
      })
      ...
      assert.equal(status.retrievalUrl, 'https://pdp.example.com/piece/' + mockPieceLink)
    ...
    it('should handle invalid PieceLink', async () => {
      ...
      try {
        await service.pieceStatus('invalid-pieceLink')
        assert.fail('Should have thrown error for invalid PieceLink')
      } catch (error: any) {
        assert.include(error.message, 'Invalid PieceLink provided')
      }
    })

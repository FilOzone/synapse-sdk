...
async function main () {
  try {
    ...
    */
    ...
    console.log('\nUpload result:')
    console.log(`  PieceLink: ${uploadResult.pieceLink}`)
    console.log(`  Size: ${formatBytes(uploadResult.size)}`)
    console.log(`  Piece ID: ${uploadResult.pieceId}`)

    // Step 7: Download the file back
    console.log('\n--- Downloading File ---')
    console.log(`Downloading piece: ${uploadResult.pieceLink}`)

    const downloadedData = await synapse.download(uploadResult.pieceLink)
    ...
    console.log('\n--- Piece Status ---')
    const pieceStatus = await storageService.pieceStatus(uploadResult.pieceLink)
    ...
    console.log(`- Piece CID / hash (PieceLink / CommP): ${uploadResult.pieceLink}`)
    console.log(`- Data set ID: ${storageService.dataSetId}`)
    console.log(`- Piece ID: ${uploadResult.pieceId}`)

...
- **Testing**: Mocha with `/* globals describe it */`, Chai for `{ assert }` in `src/test/`
  - **Sinon.js**: Test mocking framework used throughout the test suite. Key helpers in `src/test/sinon-helpers.ts`:
    - `useSinon()` - Creates sandbox with automatic cleanup
    - `createMockProvider()`, `createMockSigner()` - Mock Ethereum providers/signers
    - `createMockPandoraService()`, `createMockPDPServer()` - Service mock factories
    - `useFakeTimers()` - Control time in tests for instant execution
    - Prefer Sinon stubs/spies over manual mocks for consistency
